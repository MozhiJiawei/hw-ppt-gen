...
5. Plan the deck slide by slide before coding. Save the plan as `.tmp/<deck>/<deck>_plan.json`. Keep the plan separate from visual construction:
   ...
   - Choose one deck-level visual-anchor renderer before coding and record it once in the plan as top-level `visual_anchor_renderer: "rough_svg"` or `"ppt_native"`. Pass the same value to `createHuaweiDeck({ visualAnchorRenderer })`. Do not use environment variables for visual-anchor rendering.
...
- For 图文并茂 pages, use `contentLayout` as a layout container with `blocks`: first choose exactly one of the four fixed page layouts (`05 内容 二分栏`, `06 内容 偏分栏`, `07 内容 三分栏`, `08 内容 四分栏`), then declare one or more `visual_anchor` blocks, then add editable PPT text annotations. The schema is semantic and renderer-neutral except that `Matrix/table` is always a native PPT table and `Evidence` is always an evidence module. Image/SVG placement must use proportional contain placement only; never stretch width and height independently.
...
For `addVisualAnchorContentSlide`, pass the 24 pt title as `title` and the 18 pt explanatory subtitle as `titleNote` or `titleSubtitle`. Pass `summary` as a string, array, or `{ body/items, fill }`. Prefer `summary.body` entries as `{ label, text }`, for example `{ label: "规划先行", text: "先完成页面级观点规划，再进入生成脚本。" }`. The helper keeps the left label fixed as `分析总结`; do not use `summary.title` to replace that label. Use `visualAnchorCaption` or `visual_anchor_caption` for the editable Chinese figure-legend text below the visual anchor. This caption is PPT text-layer content and must stay outside `visual_anchor.visual_spec`. For richer pages, pass `contentLayout` with `type`, `reference`, and `modules`; each module may use `flow: "left_right"` or `flow: "top_bottom"` to select the internal 图文 relationship.
...
For structured visual anchors, use `scripts/pptx/hw_diagram_helpers.js` as the shared renderer and validator. Use `validateVisualAnchorSpec` for model-facing specs, `renderVisualAnchorPptNative` for PPT-native rendering, and SVG/image export helpers only when the deck-level `visual_anchor_renderer` is `rough_svg`. `Matrix/table` always renders through the native PPT table path. Do not create an alternate content-page renderer in deck-specific scripts; put local custom work behind `visual_anchor` data and supporting cards. Validate final deck composition through exported PNGs.
...
- Verify the plan has one top-level `visual_anchor_renderer` and no slide-level, content-layout-level, or anchor-level renderer fields.
...
- Treat `content_visual_anchor_renderer_config_missing`, `content_visual_anchor_renderer_scope_invalid`, `content_visual_anchor_layout_missing`, `content_visual_anchor_layout_invalid`, `content_visual_anchor_plan_reason_missing`, and `content_visual_anchor_table_not_native` as blockers.
