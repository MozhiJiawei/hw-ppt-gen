...
For dense text blocks, mark 1-3 decisive terms in red bold through `emphasis` in the text block. Emphasize nouns, numbers, and decisions: `资源错配`, `关键瓶颈`, `资源下降`, `不外推为保证`.

A column module should not accumulate more than 6 visible prose lines across multiple text blocks. When a column needs more detail, first split the information into KPI rows or a conclusion box. Use a compact table only when the point depends on row/column comparison.
...
- Strong: `推理收益：更小 cache 释放 batch 空间，吞吐才有上升入口。`
