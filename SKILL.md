...
4. Compose the deck before coding:
   ...
   - Use `emphasis` in text blocks for 1-3 decisive terms after the claim handle. The claim handle before `：` is a structural label: keep it bold black, not red. When layout feedback says density is low, first add or sharpen editable conclusion lines. Use KPI cards only for real metrics, and use a table only when a real two-axis relationship would be lost as prose.
