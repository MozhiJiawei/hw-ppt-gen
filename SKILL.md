...
Rough SVG visual anchors are diagram images, not miniature PPT pages. They may include diagram-native labels, axis text, values, and brief node notes required for self-explanation, but must not include page titles, slide claims, figure legends, source notes, standalone captions, or interpretation paragraphs. Put those in editable PPT text boxes, `分析总结`, supporting cards, or evidence legends. Do not use `visual_spec.annotation`.
