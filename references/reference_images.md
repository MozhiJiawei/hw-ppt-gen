# Reference Images

The bundled reference images live in `assets/slides_ref/`.

Use them before writing deck code:

1. Open the images or inspect them through the app.
2. Identify the dominant layout family needed for the current deck.
3. Match density, card rhythm, red/gray balance, table styling, and chart restraint.
4. For正文内容页, match the updated examples that place an `分析总结` band directly below the page title.
5. During visual QA, compare generated slides against these images and note any unresolved mismatch.

Reference set:

- `01 目录.png`: contents layout.
- `02 配色示例.png`: color palette and neutral/red balance.
- `03 手绘图 饼图.png`: rough SVG proportion-chart treatment for `Quantity/proportion_chart`.
- `04 手绘图 柱状图.png`: rough SVG bar-chart treatment for `Quantity/bar_chart`.
- `05 内容 二分栏.png`: equal two-column content page.
- `06 内容 偏分栏.png`: biased column layout with top analysis summary.
- `07 内容 三分栏.png`: three-column content page with top analysis summary.
- `08 内容 四分栏.png`: four-panel 2x2 content page with top analysis summary.
